...
from algorithms.utils import read_data
...
from genetic import GeneticSearch, Selection, Crossover, Mutation
from routines import path_length
    ...
    for algorithm in config:
        ...
        for combination in grid:
            ...
            for r in range(nr_tries):
                ...
                if result['best cost'] < best_result: #nadpisujemy tylko najlepsze wyniki danej  kombinacji
                    best_result = result['best cost']
                    search_summary = {str(k): str(v) for k, v in search.__dict__.items()} #zamiana na string



                    try:
                        search_summary['selection_method'] = str(search_summary['selection_method'])
                        search_summary['cx_method'] = str(search_summary['cx_method'].__str__)
                        search_summary['mut_method'] = str(search_summary['mut_method'])
...
if __name__ == '__main__':
    config = [
        ...
        {'algorithm': GeneticSearch,
         ...
         'params': {
             'data': [read_data('../data/Dane_TSP_127.xlsx')],
             'selection_method': [Selection.RANK, Selection.TOUR],
             'cx_method': [Crossover.MKX, Crossover.OX],
             'mut_method': [Mutation.INSERT, Mutation.ARBITRARY, Mutation.MASK],
             'cost_fun': [path_length],
             'generations': [50,40, 10, 20],
             'pop_size': [50, 10, 20, 40],
             'mutation_threshold': [0.01, 0.05, 0.1, 0.09],
             'crossover_mask_size': [10, 20, 15, 30],
             'mut_mask_size': [3,5,10]
         }
         }
        # ,
    ...
    filename = 'results/results_genetic_big4.json'


    results_genetic = run_grid_search(config, filename, nr_tries=2)
